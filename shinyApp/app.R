...
server <- function(input, output) {
   ...
   output$distPlot <- renderPlot({
       ...
       len = min(c(nrow(names %>% filter(Name == name)),70))
       #MJ graph
       p = ggplot(data = sansa) + geom_line(data = sansa %>% filter (Gender=="F") ,aes(x=Year, y=sum), color="hotpink1", size=1.3, show.legend = TRUE) + 
         ...
         scale_x_continuous(breaks = scales::pretty_breaks(n = len)) + 
         scale_y_continuous(breaks = scales::pretty_breaks(n = 10)) + 
         
         
         theme_minimal() + theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position = "bottom") + guides(color = guide_legend(reverse = TRUE)) +
